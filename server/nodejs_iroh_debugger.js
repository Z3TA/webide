/*
	Debugging (NodeJS) JavaScript using Iroh (https://maierfelix.github.io/Iroh/)
	
	Problem:
	How to exit a program !?
	
	Solution:
	This script is spawned by the server (running as root) on request by the user worker.
	This script runs chroot and setuid given in process.env
	server subprocess.disconnect() so that this script can exit gracefully when there is no jobs left
	All messages are passed via stdout to server.js
	
	
	Gotchas
	-------
	We are not able to capture errors from here. (they have to be handled by whoever spawns this script)
	We can however use a try catch to catch errors!
...
// For sending "special" messages to stdout
...
function runScript(scriptName) {
	...
	displayErrors: true
		...
		// Find undefined object properties
		stage.addListener(Iroh.MEMBER).on("fire", function(e) {
			//console.log(e);
			
			if (e.object[e.property] === undefined) {
				var node = e.getASTNode();
				console.log(APC + JSON.stringify(node.loc) + GS + "undefined" + GS + e.property + GS, e.object, " is undefined ");
			}
			//else console.log("hmm ? " + e.object[e.property] + " instanceOf Object = " + (e.object instanceof Object));
		});
		
		// Trigger on console.log to give proper location !?
	...
	try {
		var result = vm.runInContext(stage.script, context, scriptOptions);
	}
		catch(err) {
			console.log("vm:err:" + err.message);
			var where = getStack(scriptName);
		//process.send({error: err.message, scriptName: scriptName, location: where, stack: err.stack()});
			console.log(APC + where + GS + "error" + GS + err.message + GS + err.stack());
	}
		
		// result is always undefined ...
		// We will exit when the script exists!
