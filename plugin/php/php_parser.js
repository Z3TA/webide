(function() {
	...
	*/
	...
	editor.on("start", phpParserInit, 100);
	
	function phpParserInit() {
		...
		function onFileOpen(file) {
			...
			if(isPHP(file)) {
				...
				file.haveParsed(parsePHP(file)); // Tell the file that it has been parsed so that functions depending on the parsed data can update
	...
	function parsePHP(file) {
		...
		var xmlMode = (file.fileExtension == "php");
		...
		for(var charIndex=0; charIndex<text.length; charIndex++) {
			...
			/*
				...
				?>
				questionmark
				ps. The ending ?> can be omitted!
				
			*/
			if(!insideComment && !insideQuote) {
				if(pastChar[0] ==  && pastChar[0] ==  && pastChar[0] ==  && pastChar[0] ==  && pastChar[0] ==  && char == space) { // <?php 
					xmlMode = false;
				}
		...
		return {language: "PHP", quotes: quotes, comments: comments, globalVariables: globalVariables, blockMatch: (thisRowIndentation === 0), xmlTags: xmlTags};
	...
	function isPHP(file) {
		...
		if(file.fileExtension == "php") return true;
		
		if(file.text.match(/<\?php /i) != null) return true;
