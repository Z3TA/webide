	...
	var disabledKeyBindings = [];
		...
		console.log(inputEvent);
			...
			*/
			...
			if(input.value.charAt(0) == ":") return do_cmdline(input.value.splice(1))
			
		// if <CR> or <Esc> and :  do_cmdline
		
			var command = input.value.match(/([^\d])?(\d+)?(.)/);
			
			if(command) normal_cmd(command[1], command[2], command[3]);
		}
		
		function normal_cmd(command, number, action) {
			...
			var commandStr = input.value;
			
			// Is it allways {command}?{number}?{action} !??
			// /(.)?(\d+)?(.)
			
			var lastChar = commandStr.charAt(commandStr.length-1);
			var llChar = commandStr.length > 1 ? commandStr.charAt(commandStr.length-2) : null;
			
			var rePrecedingNr = new RegExp("(\\d*)" + lastChar);
			var matchPrecedingNr = commandStr.match(rePrecedingNr);
			var precedingNr = matchPrecedingNr ? parseInt(matchPrecedingNr[matchPrecedingNr.length-1]) : null;
			var precedingCmd = commandStr.match(/(.*)/d?.$
			
			// Motions (cursor motion command)
			
			if(lastChar == "0") {
				// Goto start of line (column 0)
			...
			if(lastChar == "h") {
				if(LAST_COMMAND == "d") {
					file.moveCaretLeft(file.caret, (precedingNr !== null) ? precedingNr : 1);
					file.deleteCharacter();
				}
				else file.moveCaretLeft(file.caret, (precedingNr !== null) ? precedingNr : 1);
				
				EDITOR.renderNeeded();
			}
			if(lastChar == "j") file.moveCaretDown(file.caret, (precedingNr !== null) ? precedingNr : 1);
