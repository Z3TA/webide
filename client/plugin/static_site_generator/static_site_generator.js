			...
			function pullFromRepo() {
				...
				function hgPull(err, resp) {
					if(err) {
						...
						summary.push("Pulled " + (changes || 0) + " changes to " + (filesChanged.length) + " files from " + repoUrl)
	...
	function newPage(site) {
		...
		EDITOR.readFromDisk(resolvePath(site, site.template), function fileRead(err, path, text) {
			...
			if(err) alertBox("Unable to find new file/page template! (site.template=" + site.template + "). " + err.message);
				...
				text = text.replace('<meta name="author" content="Jon Doe">', '<meta name="author" content="' + EDITOR.user.name + '">');
				text = text.replace('<p>Written by <a href="/" rel="author">Jon Doe</a> Mars 22, 2042.</p>', '<p>Written by <a href="/" rel="author">' + EDITOR.user.name + '</a> ' + monthName[date.getMonth()] + ' ' + date.getDate() + ', ' + date.getFullYear() + '.</p>');
