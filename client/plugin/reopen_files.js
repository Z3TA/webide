+(function() {
	...
	*/
		...
		EDITOR.localStorage.getItem("openedFiles", function(err, openedFilesString) {
			...
			*/
			...
			EDITOR.once("storageReady", function combineOpenedFilesString() {
				...
				EDITOR.localStorage.setItem("openedFiles", openedFilesString, function afterLocalStorageSet(err) {
					if(err) throw err;
				...
				function open() {
					...
					if(lastFileState) {
						...
						if( loadLastState || lastFileState.isSaved === false ) {
							...
							EDITOR.getFileHash(path, function gotHash(err, hash) {
	...
	function saveStateOfOpenFiles(reason, callback) {
		...
		//else {console.warn("reopenFiles: Unable to save opened files to server! CLIENT.connected=" + CLIENT.connected + "  EDITOR.storage.ready()=" + EDITOR.storage.ready());}
		...
		EDITOR.localStorage.getItem("openedFiles", function(err, openedFilesString) {
			if(openedFilesString == null || openedFilesString == "") {
				...
				findBugs(true, function(err, openedFilesString) { // true == also check if the list match EDITOR.files
					if(err) {
						console.warn("reopenFiles:saveStateOfOpenFiles: found bug! err=" + err.message);
	...
	function saveState(path, callback) {
		...
		//else {console.warn("reopenFiles: Not saving state for " + file.path + " because it has " + file.text.length + " (over " + (sizeLimit-1) + ") characters");}
	...
	function findBugs(checkMatch, callback) {
		...
		EDITOR.localStorage.getItem("openedFiles", function afterLocalStorageGet(err, text) {
			if(err) throw err;
			if(text == null) text = "";
